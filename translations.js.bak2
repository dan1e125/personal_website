...
const translations = {en:{documentTitle:'Daniel Ordonez Arango | Penetration Tester',metaDescription:'Freelance penetration tester for startups and SMBs. Web app, Active Directory & network security. HTB Top 1%, 702 lab targets. Remote-first, clear deliverables.',ogTitle:'Daniel Ordonez Arango | Penetration Tester',ogDescription:'Freelance pentester for startups and SMBs. Web app, AD & network pentest. HTB Top 1%, 702 lab targets. Remote engagements, clear reporting.',selectors:{'.topnav a[href="#about"]':'About','.topnav a[href="#certifications"]':'Credentials','.topnav a[href="#contact"]':'Contact','.hero-copy .lead':'I help startups and SMBs find exploitable vulnerabilities before attackers do. Simple, end-to-end process — I handle everything. You get clear findings, real risk ratings, and a report your team can start fixing the same day.','#spec-web':'Web Pentest','#spec-ai':'AI / LLM Security','#spec-ad':'Active Directory','#spec-net':'Network Pentest','.hero-actions .button.primary':'Start with a free call','#stat-paths':'HTB paths completed','#stat-targets':'HTB machines pwned','#stat-ranking':'HTB ranking',
  ...
  formMessages:{
    ...
    {cmd:'gobuster dir -u https://target.tld -w raft-medium.txt',lines:['/admin        (301)  [→ /admin/login]','/backup       (200)  [Size: 4.8 KB]  ← exposed!','/api/v1       (200)  [Size: 1.3 KB]','[+] Sensitive path found → data exposure risk']},
    {cmd:'nmap -sV -p 22,443,3389 10.10.14.0/24',lines:['PORT      STATE  SERVICE        VERSION','22/tcp    open   OpenSSH 7.6','3389/tcp  open   ms-wbt-server  ← RDP exposed','[+] Attack surface mapped: RDP + SSH on subnet']},
    {cmd:'crackmapexec smb 10.10.10.0/24 -u users.txt -p pass.txt',lines:['SMB  10.10.10.5  [*] Windows 10 (domain: CORP)','SMB  10.10.10.5  [+] CORP\\svc_backup:Password123!','SMB  10.10.10.5  [+] Pwn3d!  ← admin share access','[+] Domain Admin obtained → pivoting...']},
    {cmd:'python3 llm_probe.py --model gpt-4o --test all',lines:['[→] Testing: prompt injection...','[+] System prompt leaked (45 tokens)','[→] Testing: jailbreak via role-play...','[+] Safety bypass confirmed (CVSS 8.1)']}
  ],
  ...
  selectors:{
    ...
    '#hero-role':'Penetration Tester','.hero-copy h2':'Detecta el fallo antes de que sea un problema.',
    ...
    '#qcs-price-pentest_ai':'Desde $2,000','#quote-heading':'Obtén una estimación al instante','#quote-copy':'Completa los detalles para obtener un rango de precio al instante. Se enviará una propuesta formal después de la llamada de alcance.','#ql-service':'Servicio','#terminal-panel-label':'[ PANEL DE ESTADO ]','#svc1-price':'Desde $2,000 USD','#svc2-price':'Desde $3,000 USD','#svc3-price':'Desde $4,000 USD','#svc4-price':'Desde $2,000 USD','#ql-env-params':'Parámetros del entorno','#tgl-small-label':'Pequeño','#tgl-small-desc':'1–5 objetivos','#tgl-medium-label':'Mediano','#tgl-medium-desc':'6–15 objetivos','#tgl-large-label':'Grande','#tgl-large-desc':'15+ objetivos','#tgl-low-cplx':'Estándar','#tgl-low-cplx-desc':'Stack común','#tgl-medium-cplx':'Personalizado','#tgl-medium-cplx-desc':'Lógica propietaria','#tgl-high-cplx':'Empresarial','#tgl-high-cplx-desc':'Complejo / híbrido','#tgl-black-tbox':'Caja negra','#tgl-black-tbox-desc':'Sin acceso previo','#tgl-grey-tbox':'Caja gris','#tgl-grey-tbox-desc':'Credenciales parciales','#tgl-white-tbox':'Caja blanca','#tgl-white-tbox-desc':'Acceso completo + código','#ql-scope':'Tamaño del alcance','#ql-complexity':'Complejidad','#ql-tbox':'Tipo de testing','#contact .section-tag':'[ CONTACTO ]',
  ...
  terminalSessions:[
    {cmd:'gobuster dir -u https://target.tld -w raft-medium.txt',lines:['/admin        (301)  [→ /admin/login]','/backup       (200)  [Size: 4.8 KB]  ← expuesto!','/api/v1       (200)  [Size: 1.3 KB]','[+] Ruta sensible encontrada → riesgo de exposición']},
    {cmd:'nmap -sV -p 22,443,3389 10.10.14.0/24',lines:['PORT      STATE  SERVICE        VERSION','22/tcp    open   OpenSSH 7.6','3389/tcp  open   ms-wbt-server  ← RDP expuesto','[+] Superficie de ataque: RDP + SSH en subred']},
    {cmd:'crackmapexec smb 10.10.10.0/24 -u users.txt -p pass.txt',lines:['SMB  10.10.10.5  [*] Windows 10 (dominio: CORP)','SMB  10.10.10.5  [+] CORP\\svc_backup:Password123!','SMB  10.10.10.5  [+] Pwn3d!  ← acceso admin','[+] Domain Admin obtenido → pivotando...']},
    {cmd:'python3 llm_probe.py --model gpt-4o --test all',lines:['[→] Probando: prompt injection...','[+] System prompt filtrado (45 tokens)','[→] Probando: jailbreak via role-play...','[+] Bypass de seguridad confirmado (CVSS 8.1)']}
  ],
